...
export const createResource = async (name) => {
  ...
  try {
    ...
    populateModel({ name, capitalizeName, dbType: projectInfo.database });
    populateValidation({ name, capitalizeName, dbType: projectInfo.database });
    populateController({ name, capitalizeName, dbType: projectInfo.database });
  } catch (error) {
